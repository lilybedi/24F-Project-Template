...
@advisors.route('/student/<int:student_id>/eligibility', methods=['PUT'])
def update_student_eligibility(student_id):
    """Update student eligibility status (Story 2)"""
    try:
        ...
        '''
        cursor = db.get_db().cursor()
        cursor.execute(query, (data['eligible'], student_id))
        db.get_db().commit()
        return jsonify({"message": "Student eligibility updated"}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
@advisors.route('/positions/filled', methods=['GET'])
def get_filled_positions(advisor_id):
    """Get information about filled positions (Story 4)"""
    try:
        query = '''
            SELECT 
                p.ID, p.Name, p.Title, c.Name as Company_Name,
                p.Filled, p.Date_Start, p.Date_End,
                COUNT(DISTINCT a.ID) as Total_Applications,
...
def filter_students_by_status(advisor_id):
    ...
    try:
        hired = request.args.get('hired', type=bool)
        query = '''
            SELECT 
